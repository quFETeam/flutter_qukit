...
class BottomDragWidget extends StatelessWidget {
  ...
  BottomDragWidget({Key key, @required this.body, @required this.dragContainer, @required this.bottomBar})
      : assert(body != null),
...
GlobalKey<_DragContainerState> childKey = GlobalKey();
...
class _DragContainerState extends State<DragContainer> with TickerProviderStateMixin {
  AnimationController animalController;//动画控制器
  double maxOffsetDistance; //滑动位置超过这个位置，会滚到顶部；小于，会滚动底部。
  ...
  void initState() {
    ...
    maxOffsetDistance = (widget.height + widget.defaultShowHeight) * 0.15;
